...
$$ f_{l2fib}(x) = n * \frac{8}{2 * 64} = n * \frac{1}{16}  $$
...
As the increase of layer 3 data cache misses around the layer 3 cache
upper bound mark indicates, the throughput actually decreases at those
predicted points because of the respective caches.
...
different l2fib sizes. It's maximum size is remarkably smaller than
the layer 2 \Ac{fib} and also each lookup table key and value together
are only 64 bit in size. Assuming the table only contains keys and
values, the maximum \Ac{fib} size fitting into cache of $n$ bytes is:

$$ f_{ip6fib}(x) = n * \frac{8}{2 * 32} = n * \frac{1}{8}  $$

This means for a layer 1 cache size of 32KB ($2^{15}$) at most 4096
could fit into it. At this \Ac{fib} size there is no change in
throughput, nor a drastic change in layer 1 data cache load misses.

% TODO there is a change before 4096 in omastar 

% l2 cache size of 256k (2^18) at most 32,768 routes -> right after the big drop

Nevertheless it can be assumed the massive drop in performance upwards
from 20,000 \Ac{fib} entries is because of the BiHash table and the
table which correlates the BiHash table result to actual information
are not efficiently accessible anymore for traffic with random
destinations.
